...
class AirQualityViewModel {
    ...
    func fetchCoordinates(city: String, completion: @escaping (Double, Double) -> Void) {
        ...
        let urlString = "https://api.opencagedata.com/geocode/v1/json?q=\(encodedCity)&key=2cf8d24ae52349ef811a6ff1fdd4c3fe"
    ...
    func fetchAirQualityInfo(lat: Double, lon: Double) {
        let urlString = "https://api.airvisual.com/v2/nearest_city?lat=\(lat)&lon=\(lon)&key=98d98bf5-d9c8-4644-aefb-e52bf4733502"
        guard let url = URL(string: urlString) else {
            showError?("Invalid URL")
            return
        }
